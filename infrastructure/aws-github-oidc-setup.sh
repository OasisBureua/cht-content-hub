...
echo "Setting up GitHub Actions OIDC for Content Hub AWS deploy"
echo "========================================================="
echo ""

read -p "GitHub org or user: " GITHUB_USER
read -p "Repository name [cht-content-hub]: " REPO_NAME
...
ROLE_NAME="GitHubActions-ContentHub"
...
sed "s/ACCOUNT_ID/${AWS_ACCOUNT_ID}/g" "$POLICY_DOC" > /tmp/contenthub-deploy-policy.json

POLICY_ARN=$(aws iam create-policy \
  --policy-name "$POLICY_NAME" \
  --policy-document file:///tmp/contenthub-deploy-policy.json \
  --query 'Policy.Arn' \
  --output text 2>/dev/null || \
...
rm -f /tmp/github-trust-policy.json /tmp/contenthub-deploy-policy.json
