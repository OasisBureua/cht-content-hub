...
#
# Prerequisite: ./scripts/deploy-primary.sh prod with deploy_api_ecs_service = false
#
# Usage:
...
GITHUB_VAR_FILE="$REPO_ROOT/infrastructure/terraform/environments/variables/${ENV}.github.tfvars"
BACKEND_CONFIG="$REPO_ROOT/infrastructure/terraform/environments/backends/us-east-1-${ENV}.hcl"

if [ -f "$VAR_FILE" ]; then
  TFVARS="$VAR_FILE"
elif [ -f "$GITHUB_VAR_FILE" ]; then
  TFVARS="$GITHUB_VAR_FILE"
else
  echo "Missing $VAR_FILE (copy from ${ENV}.tfvars.example)"
  exit 1
fi
...
API_DOMAIN="$(grep -E '^api_domain[[:space:]]*=' "$TFVARS" | head -1 | sed -E 's/^[^"]*"([^"]*)".*/\1/')"
