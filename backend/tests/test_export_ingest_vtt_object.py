...
FILE_STEM = "file1"
KEY = f"zoom-recordings/{PROGRAM_ID}/{MEETING_ID}/{FILE_STEM}.vtt"
...
def test_decode_object_key_unquotes():
    encoded = "zoom-recordings/prog%2Fid/meet/file.vtt"
    assert decode_object_key(encoded) == "zoom-recordings/prog/id/meet/file.vtt"


def test_parse_vtt_object_key():
    assert parse_vtt_object_key(KEY) == (PROGRAM_ID, MEETING_ID, FILE_STEM)
    assert parse_vtt_object_key("zoom-recordings/x/y/file.mp4") is None
    assert parse_vtt_object_key("session-heroes/x/y/file.vtt") is None
    assert parse_vtt_object_key("zoom-recordings/only-two/parts.vtt") is None
...
async def test_unlinked_skips_without_insert(db_session: AsyncSession):
    ...
    result = await apply_vtt_object(db_session, store, key)
    assert result.status == "skipped_unlinked"
